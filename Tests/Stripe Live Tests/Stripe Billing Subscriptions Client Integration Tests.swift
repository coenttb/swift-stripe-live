...
struct SubscriptionClientTests {
    ...
    func testCreateSubscription() async throws {
        ...
        // Verify subscription
        #expect(response.status == Stripe.Billing.Subscription.Status.active || response.status == Stripe.Billing.Subscription.Status.incomplete)
    ...
    func testListSubscriptions() async throws {
        ...
        let response = try await client.client.list(.init(
...
//
    ...
    func testCancelSubscription() async throws {
        ...
        let canceled = try await client.client.cancel(created.id, .init(
            invoiceNow: true,
            prorate: true
        ))
        ...
        #expect(canceled.status == Stripe.Billing.Subscription.Status.canceled || canceled.status == Stripe.Billing.Subscription.Status.incompleteExpired || canceled.canceledAt != nil || canceled.cancelAtPeriodEnd == true)
...
//
    ...
    func testSubscriptionWorkflow() async throws {
        ...
        #expect(created.status == Stripe.Billing.Subscription.Status.active || created.status == Stripe.Billing.Subscription.Status.incomplete)
