...
struct StripeRateLimitStressTests {
    ...
    func testExceedingStripeLimits() async throws {
        ...
        for i in 1...requestsPerSecond {
            ...
            do {
                ...
                if wasDelayed {
                    delayedRequests += 1
                    print("Request \(i): SUCCESS (delayed: \(String(format: "%.3f", requestDuration))s)")
                } else {
                    print("Request \(i): SUCCESS (fast: \(String(format: "%.3f", requestDuration))s)")
                }
        ...
        print("Effective rate: \(String(format: "%.1f", effectiveRequestsPerSecond)) requests/second")
        print("===========================\n")
        
        // Analysis
        if delayedRequests > 0 {
            let avgDelay = requestTimings.filter { $0.delayed }.map { $0.duration }.reduce(0, +) / Double(delayedRequests)
            print("Average delay for throttled requests: \(String(format: "%.3f", avgDelay)) seconds")
        }
        ...
        #expect(effectiveRequestsPerSecond <= 26, "Effective rate should not exceed Stripe's limit (with small tolerance)")
    }
    ...
    func testConcurrentBurstWithinLimits() async throws {
        ...
        #expect(successCount == concurrentRequests, "All 25 concurrent requests should succeed at the limit")
    }
    ...
    func testRateLimitingBehaviorOverTime() async throws {
        ...
        print("Effective rate: \(String(format: "%.1f", Double(requestCount) / totalDuration)) req/s")
        print("=======================\n")
