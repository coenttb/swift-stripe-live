...
struct StripeRateLimitWithinLimitsTests {
    ...
    func testRequestsWithinLimits() async throws {
        ...
        #expect(normalRequests > delayedRequests, "Most requests should have normal latency when within rate limits")
        #expect(avgDuration < 0.5, "Average request duration should be under 500ms when not rate limited")
    }
