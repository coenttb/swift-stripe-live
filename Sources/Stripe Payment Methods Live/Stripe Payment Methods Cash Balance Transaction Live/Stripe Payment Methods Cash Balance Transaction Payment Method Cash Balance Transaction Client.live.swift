extension Stripe.PaymentMethods.PaymentMethods.Client {
    public static func live(
        makeRequest: @escaping @Sendable (_ route: Stripe.PaymentMethods.PaymentMethods.API) throws -> URLRequest
    ) -> Self {
        ...
        return Self(
            ...
            retrieveCustomer: { customerId, paymentMethodId in
                try await handleRequest(
                    for: makeRequest(.retrieveCustomer(customerId: customerId, paymentMethodId: paymentMethodId)),
                    decodingTo: Stripe.PaymentMethods.PaymentMethod.self
                )
            },
