...
extension URLRequest.Handler.Stripe: DependencyKey {
    ...
    private static func performRateLimitedRequest(
        ...
        if !acquisitionResult.canProceed {
            ...
            // Retry after waiting
            return try await performRateLimitedRequest(request, retryCount: retryCount, maxRetries: maxRetries)
        }
            ...
            if let httpResponse = response as? HTTPURLResponse {
                if httpResponse.statusCode == 429 {
                    ...
                    guard retryCount < maxRetries else {
                        ...
                        throw URLError(.dataNotAllowed, userInfo: [
                            NSLocalizedDescriptionKey: "Rate limit exceeded after \(maxRetries) retries (reason: \(reasonDescription))"
                        ])
                    }
